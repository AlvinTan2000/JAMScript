...
class JAMDatasource {
    ...
    addDataStream(deviceID) {
        this[this.numDataStreams] = new JAMDatastream(deviceID, this.buildKey(deviceID), this.jammanager, this);
    ...
    // TODO: Consider better alternatives
    static listenerEvent(pattern, channel, bufferData) {
        let data = bufferData.toString();
        // If there was data and the associated JAMDatastream is not local
        if (data !== undefined && !JAMDatastream.jDataStreamList[data].isLocalStream) {
            // If the associated JAMDataStream exist to begin with, query the new data
