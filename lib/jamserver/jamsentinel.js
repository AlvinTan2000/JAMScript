...
class JAMSentinel {
    ...
    setSentinel(host, port, callback) {
        ...
        this.rSenListener.subscribe("+failover-detected")
        this.rSenListener.subscribe("+switch-master")
        this.rSenListener.on("message", (channel, message) => {
            if (message.indexOf("+failover-detected") != 0) {
                self._failoverDetected(channel, message);
            } else {
                self._failoverHandler(channel, message).bind(self)
            }
        })
    ...
    /**
     * Simple handling upon detection of a failover
     * @param channel : String
     * @param message : String
     * @private
     */
    _failoverDetected(channel, message, callback) {
        if (callback){
            callback();
        }
    }
