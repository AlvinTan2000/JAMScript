...
class JAMRedis {
    ...
    logData(key, timestamp, entry, callback, isLocal) {
        ...
        if (isLocal) {
            ...
            this.rLocalPipe.xadd([key, timestamp, timestamp, cbor.encode(entry)], cb);
                ...
                if (error) {
                    ...
                    if (result !== null) {
                        ...
                        for (let i = 0; i < result.length; i++) {
                            let timestamp = result[i][0].toString();      // Get the ID
                            let log = cbor.decodeFirstSync(result[i][1][1]).value;         // Get the value
