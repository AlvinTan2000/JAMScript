...
class JAMDatastream {
    ...
    constructor(dev_id, key, fresh, jammanager, data_prototype, refresh_rate, slots, redis) {
        ...
        this.set_size = 0;
        this.fresh = fresh;
        this.index_of_last_value = 0;               // To ZRANGE without having to retrieve everything
        ...
        // Refresh Rate = how frequent to refresh the Jdata value
        // 0 < = No Refresh values
        //  0  = refresh as event arrives
        // > 0 = refresh every miliseconds
    ...
    /* Returns the last data pair (Number, Date) in the data stream or null if the data stream is empty */
    lastData() {
        if (!this.isEmpty()) {
    ...
    /* Returns the last value (of type Number) in the data stream or null if the data stream is empty */
    lastValue() {
        if (!this.isEmpty()) {
            var data = this.data_values[this.size() - 1], value;
            // if the content of the string is a number
    ...
    */
    ...
    log(entry, callback, redis) {
        ...
        var datatype = this.datasource.getDataType();
        ...
        if (datatype) {
            ...
            this.datasource.setDataType(entryType);
        }
