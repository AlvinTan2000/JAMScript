...
class JAMDatastream {
    ...
    constructor(dev_id, key, fresh, jammanager, data_prototype, refresh_rate, slots, redis) {
        ...
        // Associated Objects
        this.listerners_list = [];                       // TODO: What does this store
    ...
    getLastNData(N) {
        ...
        return this.getAllData().slice(this.size() - N);
    }
     ...
     * @return {number[]}
     */
    getLastNDataValues(N) {
        return this.getLastNData(N).map(
            function (data) {
                return data.value;
            });
    }
     ...
     * @return {number[]}
     */
    getValuesAfter(timestamp) {
        return this.getDataAfter(timestamp).map(function (data) {
            return data.value;
        });
    }
     ...
     * @return {{value: number, timestamp: Date}[]}
     */
    getDataBetween(fromTimestamp, toTimestamp) {
        return this.getAllData().filter(function (data) {
     ...
     * @return {number[]}
     */
    getDataValuesBetween(fromTimestamp, toTimestamp) {
        return this.getDataBetween(fromTimestamp, toTimestamp).map(function (data) {
            return data.value;
        });
    }


    // TODO: Difference between the following and the previous
    ...
    request_set_size() {
        this.redis.zcard(this.key,
            function (error, response) {
                ...
                this.set_size = response;
                if (this.fresh) {
                    this.index_of_last_value = response - 1;
                }
            }
        );
    }
    ...
    // TODO: What is request value refresh

    // TODO: Unsused Boolean
    request_value_refresh(force) {
        ...
        // ZRANGE from last queried index to the latest (ie. -1)
        this.redis.zrange(this.key, this.lastValueIndex, -1,
            function (e, response) {
                process(e, response, datastream);
            });
    ...
    process_zrange_response(error, response, datastream) {
        ...
        if (error) {
            ...
            for (let i = 0; i < response.length; i++) {
                ...
                for (let listener of datastream.listerners_list) {
    ...
    log(entry, callback, redis) {
        ...
        // TODO: Some concerns with error checking here
        ...
        redis.zadd([this.key, curTime, cbor.encode(entryObject)],
            function (error, response) {
                if (error) {
                    ...
                    // Perform data regularization in case the addition exceeded the maximum entries allowed
    ...
    // TODO: What is this and the next?
    ...
    performDataRegularization() {
        ...
        if (this.space > 0 && this.jamdatastream.length > this.space) {
            ...
            this.redis.zremrangebyscore([this.key, from, to], function (error) {
                if (error) {
                    if (debug) {
                        console.log("ERROR", resp.error);
                    }
    ...
    subscribe(listener) {
        this.listerners_list.push(listener);
    }

    unsubscribe(listener) {
        // Iterate through to find the specific listener and remove it from the list
        for (let i = 0; i < this.listerners_list.length; i++) {
            if (this.listerners_list[i] === listener) {
                this.listerners_list.splice(i, 1);
                break;
            }
        }
    }
